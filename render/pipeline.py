...
frame, calls annotation.annotate_frame, and pipes the annotated frames
directly into ffmpeg for H.264 encoding -- single pass, single output file.
...
Single-pass encoding: frames are written directly to an ffmpeg subprocess
over stdin as raw BGR24 video and encoded straight to H.264 -- no
intermediate mp4v file, no separate decode+re-encode transcode step. This
replaces the old two-stage approach (cv2.VideoWriter to a raw .mp4, then a
second ffmpeg pass to transcode it), which wrote and then fully re-decoded
every frame a second time before the final encode could even start.
"""
...
class RenderPipeline:
    ...
    def render(
        ...
        Writes ONE output file, in a single pass: each annotated frame is
        piped straight into ffmpeg over stdin (raw BGR24) and encoded
        directly to H.264 with +faststart for browser/Streamlit playback.
        No intermediate file, no separate transcode step."""
        cfg = self.config
            ...
            with tqdm(total=total, desc="Rendering annotated video", unit="frame") as pbar:
                while True:
                    ...
                    annotate_frame(
                        ...
                        team_colors=team_colors or {0: (179, 0, 0), 1: (110, 238, 255)},
        ...
        output_lines = ffmpeg_proc.stdout.read().decode(errors="replace")
